  ...
  return (
    <tr>
      <th className={styles.cell}>
        <NavLink className={styles.link} to={`/admin/institutes/${row.id}`} >Редактировать</NavLink>
      </th>
      <th className={styles.cell}>{row.id}</th>
      <th className={styles.cell}>{row.name}</th>
      <th className={styles.cell}>{row.rating}</th>
      <th className={styles.cell}>
        <NavLink className={styles.link} to={`/admin/institutes/${row.id}`} >Перейти к рейтингу</NavLink>
      </th>
    </tr>
  );
};
